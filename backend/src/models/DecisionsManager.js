...
class DecisionsManager extends AbstractManager {
  ...
  readfilter(
    ...
    if (userExpert !== "0") {
      queryContent = `SELECT ${this.table}.id, ${this.table}.content, ${this.table}.status, ${this.table}.date_created, ${this.table}.date_update, users.firstname, users.lastname FROM ${this.table} INNER JOIN users WHERE ${this.table}.id_user_creator = users.id AND ${this.table}.id IN (SELECT decisions_experts.id_decisions FROM decisions_experts WHERE decisions_experts.id_user_expert = ${userExpert} GROUP BY id_decisions) AND status ${operator} ? ${durees};`;
    }
    if (groupImpacted !== "0") {
